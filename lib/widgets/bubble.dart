...
class BubbleState extends State<Bubble> with SingleTickerProviderStateMixin {
              ...
              builder: (BuildContext context, Widget child) {
                return Positioned(
                  left: controller.isAnimating
                      ? animation.value * (widget._barWidth - widget._bubbleWidth)
                      : _pointerRatio * (widget._barWidth - widget._bubbleWidth),
  ...
  void animate(double end) {
    ...
    controller.duration = Duration(milliseconds: ((end - begin).abs() * 2000).toInt());
  ...
  int _getGaugeIndex() {
    ...
    var sumRatio = (1 + widget._verticalFactor * widget._cellDataList[0].rightGap) / listCount;
    var i = 0;
...
class BubblePainter extends CustomPainter {
  ...
  BubblePainter(this._ratio, this._bubbleWidth, this._bubbleHeight, this._bubbleColor, this._strokeWidth,
      this._strokeColor, this._arrowWidth, this._arrowHeight, this._pointBot);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Path path;
    path = _pointBot ? _getPointBotBubblePath(arrowPositionX) : _getPointTopBubblePath(arrowPositionX);
